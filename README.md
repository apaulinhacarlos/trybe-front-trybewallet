...
Neste projeto eu desenvolvi uma aplicação em React usando Redux como ferramenta de manipulação de estado. A aplicação é uma carteira de controle de gastos com conversor de moedas, ao utilizar essa aplicação um usuário deverá ser capaz de:
...
```bash
git clone git@github.com:apaulinhacarlos/trybe-front-trybewallet/.git
```

Acesse o diretório do projeto com o comando `cd`:

```bash
cd trybe-front-trybewallet
```
...
A página _web_ consome os dados da API do _awesomeapi API de Cotações_ para realizar a busca de câmbio de moedas, através do seguinte _endpoint_:
